...
import { Trash2, Plus, Edit, Loader2, Search, HelpCircle } from "lucide-react";
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
const SliderProductsManager = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    slider_max: "60",
    slider_step: "1",
    slider_default_start: "1",
    slider_default_end: "60",
    slider_start_label: "Start Value",
    slider_end_label: "End Value",
  ...
  const loadData = async () => {
    try {
      ...
      if (productsRes.data) {
        const multiRangeProducts = (productsRes.data as any[]).filter(
          p => p.slider_config?.slider_type !== "single"
        );
        setProducts(multiRangeProducts);
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const sliderConfig: SliderConfig = {
      ...
      default_start: Number(formData.slider_default_start),
      default_end: Number(formData.slider_default_end),
      start_label: formData.slider_start_label,
      end_label: formData.slider_end_label,
    ...
    try {
      if (editingProduct) {
        ...
        toast({
          title: "Success",
          description: "Slider product updated successfully",
        });
      } else {
        ...
        toast({
          ...
          description: "You can now generate FAQs and rewards for this product",
        });
        
        // Keep dialog open and switch to edit mode to enable FAQ/Rewards generation
        if (data) {
          setEditingProduct(data as unknown as SliderProduct);
          loadData();
          return;
        }
      }
  ...
  const handleEdit = (product: SliderProduct) => {
    ...
    setFormData({
      ...
      delivery_value: productAny.delivery_value || "order completion",
      // SEO fields
    ...
    setPricingBrackets(product.slider_config?.pricing_brackets || [{ start: 1, end: 10, price: 10 }]);
    setDialogOpen(true);
  };
  ...
  const handleAIFieldsGenerated = async (fields: GeneratedFields) => {
    console.log('SliderProductsManager - handleAIFieldsGenerated called with:', fields);
    ...
    try {
      ...
      console.log('SliderProductsManager - Current category_id:', currentCategoryId);
      ...
      if (finalSlug && currentCategoryId) {
        try {
          ...
          console.log('SliderProductsManager - Generated unique slug:', finalSlug);
        } catch (slugError) {
          console.error('SliderProductsManager - Error generating slug:', slugError);
      ...
      setFormData(prev => {
        ...
        console.log('SliderProductsManager - Updating form data to:', newFormData);
        return newFormData;
      });

      // Handle slider config if provided
      if (fields.slider_config) {
        ...
        setFormData(prev => ({
          ...
          slider_default_start: (sc.default_start || sc.default_value)?.toString() || prev.slider_default_start,
          slider_default_end: sc.default_end?.toString() || prev.slider_default_end,
          slider_start_label: sc.start_label || sc.value_label || prev.slider_start_label,
          slider_end_label: sc.end_label || prev.slider_end_label,
      ...
      console.log('SliderProductsManager - Form data updated successfully');
    } catch (error) {
      console.error('SliderProductsManager - Error in handleAIFieldsGenerated:', error);
      toast({
        title: "Error",
    ...
    <Card>
      <CardHeader className="flex flex-col space-y-4">
        <div className="flex flex-row items-center justify-between">
          <CardTitle>Slider Products Manager</CardTitle>
          <Dialog open={dialogOpen} onOpenChange={(open) => {
            ...
            >
              <DialogHeader>
                <DialogTitle>
                  {editingProduct ? "Edit Slider Product" : "Create Slider Product"}
                </DialogTitle>
              </DialogHeader>
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setFormData({ ...formData, start_time_text: e.target.value })}
              ...
              <div className="border-t pt-4">
                <h3 className="font-semibold mb-4">Slider Configuration</h3>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                  ...
                  <div className="col-span-2">
                    ...
                    <p className="text-xs text-muted-foreground mt-2">
                      Example: Level 1-10 costs $10, 11-20 costs $15, etc. Total price = sum of all brackets covered.
                    </p>
                  </div>

                  <div>
                    <Label htmlFor="slider_default_start">Default Start *</Label>
                    <Input
                      id="slider_default_start"
                      type="number"
                      value={formData.slider_default_start}
                      onChange={(e) => setFormData({ ...formData, slider_default_start: e.target.value })}
                      required
                    />
                  </div>

                  <div>
                    <Label htmlFor="slider_default_end">Default End *</Label>
                    <Input
                      id="slider_default_end"
                      type="number"
                      value={formData.slider_default_end}
                      onChange={(e) => setFormData({ ...formData, slider_default_end: e.target.value })}
                      required
                    />
                  </div>

                  <div>
                    <Label htmlFor="slider_start_label">Start Label *</Label>
                    <Input
                      id="slider_start_label"
                      value={formData.slider_start_label}
                      onChange={(e) => setFormData({ ...formData, slider_start_label: e.target.value })}
                      required
                      placeholder="e.g., My level"
                    />
                  </div>

                  <div>
                    <Label htmlFor="slider_end_label">End Label *</Label>
                    <Input
                      id="slider_end_label"
                      value={formData.slider_end_label}
                      onChange={(e) => setFormData({ ...formData, slider_end_label: e.target.value })}
                      required
                      placeholder="e.g., I want to be"
                    />
                  </div>

                  <div className="col-span-2">
                    <Label htmlFor="slider_time_per_step">Estimated Time Per Step (days) *</Label>
                    <Input
        ...
        <div className="relative max-w-md">
          ...
          <Input
            type="text"
            placeholder="Search slider products..."
            value={searchQuery}
            ...
            {filteredProducts.length === 0 && searchQuery ? (
              <TableRow>
                <TableCell colSpan={6} className="text-center text-muted-foreground">
                  No slider products match your search
                </TableCell>
              </TableRow>
            ) : filteredProducts.length === 0 ? (
              <TableRow>
                <TableCell colSpan={6} className="text-center text-muted-foreground">
                  No slider products yet. Create one to get started.
                </TableCell>
              </TableRow>
            ) : (
              filteredProducts.map((product) => {
                ...
                return (
                  <TableRow key={product.id}>
                    ...
                    <TableCell>
                      {product.slider_config?.pricing_brackets && product.slider_config.pricing_brackets.length > 0 ? (
                        <div className="text-sm">
                          <div className="font-medium">{product.slider_config.pricing_brackets.length} brackets</div>
                          <div className="text-xs text-muted-foreground">
                            ${product.slider_config.pricing_brackets.reduce((sum, b) => sum + b.price, 0).toFixed(2)} total
                          </div>
                        </div>
                      ) : (
                        <div className="text-sm">${product.slider_config?.price_per_step}/step</div>
                      )}
                    </TableCell>
                    <TableCell>
...
export default SliderProductsManager;