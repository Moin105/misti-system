-- Update process_referral_reward to accept optional p_referrer_id as fallback
...
BEGIN
  ...
  PERFORM log_security_event(
    ...
    p_referee_id,
    jsonb_build_object('order_id', p_order_id, 'order_amount', p_order_amount, 'fallback_referrer_id', p_referrer_id)
  );

  -- Get referrer from profile OR use provided fallback
  SELECT COALESCE(referred_by, p_referrer_id) INTO v_referrer_id 
  FROM profiles WHERE id = p_referee_id;
  
  -- If still NULL, try the order's referrer_id as last resort
  IF v_referrer_id IS NULL THEN
    SELECT referrer_id INTO v_referrer_id FROM orders WHERE id = p_order_id;
  END IF;
