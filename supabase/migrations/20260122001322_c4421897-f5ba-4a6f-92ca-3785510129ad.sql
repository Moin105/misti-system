-- Add unique index on coupon_usage(order_id) to prevent duplicate coupon usage per order
CREATE UNIQUE INDEX IF NOT EXISTS coupon_usage_order_id_unique ON coupon_usage(order_id);

-- Update process_order_cashback to check for existing transactions
...
BEGIN
  -- IDEMPOTENCY CHECK: Check if cashback already processed for this order
  SELECT EXISTS(
  ...
  IF v_already_processed THEN
    PERFORM log_security_event(
      ...
      p_user_id,
      jsonb_build_object('order_id', p_order_id, 'reason', 'already_processed')
    );
    RETURN jsonb_build_object(
      'success', false, 
      'reason', 'Cashback already processed for this order',
      'order_id', p_order_id
    );
  END IF;
...
-- Update process_referral_reward to check for existing order-level transactions
CREATE OR REPLACE FUNCTION public.process_referral_reward(p_order_id uuid, p_referee_id uuid, p_order_amount numeric, p_referrer_id uuid DEFAULT NULL::uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
DECLARE
  ...
  v_order_reward_exists BOOLEAN;
BEGIN
  -- IDEMPOTENCY CHECK: Check if referral reward already processed for THIS ORDER
  SELECT EXISTS(
    SELECT 1 FROM cashback_transactions 
    WHERE order_id = p_order_id 
    AND description LIKE 'Referral reward%'
  ) INTO v_order_reward_exists;
  
  IF v_order_reward_exists THEN
    PERFORM log_security_event(
      'process_referral_reward_duplicate',
      p_referee_id,
      jsonb_build_object('order_id', p_order_id, 'reason', 'already_processed_for_order')
    );
    RETURN jsonb_build_object('processed', false, 'reason', 'Referral reward already processed for this order');
  END IF;
  ...
  -- Record as cashback transaction (use 'earned' type as per constraint)
  INSERT INTO cashback_transactions (user_id, order_id, amount, transaction_type, description)
  VALUES (v_referrer_id, p_order_id, v_reward_amount, 'earned', 'Referral reward for new customer');
